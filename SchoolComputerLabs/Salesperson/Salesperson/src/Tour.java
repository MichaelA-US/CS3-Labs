...
public class Tour {
	...
	public void insertSmallest(Point p) {
		...
		{
			head = new Node(p); //instantiating new node for points
			head.next = null;
		}
		else
		{
			...
			while(store != null) //iterating through linked list
			{
				Node storeNext = store.next; //getting each next node 
				point.next = storeNext; //getting the next point of that node
				store.next = point;
				double distance = this.distance();
				if(distance < smallDist)  //checking if the distance is the smallest
				{
					smallNode = nextNode; //storing the nodes
					smallDist = distance;
				}
				store.next = storeNext;
				nextNode++; //going to the next nodes
				store = store.next;
		}
			store = this.head;
			Node tempNext = store.next;
			while(smallNode != 0) //iterating through the inidices
			{
				store = store.next;
				tempNext = store.next; //storing the nodes
				smallNode--;
			}
			point.next = tempNext;
			store.next = point;
